...
constexpr int NR = ndof  + 6+21;    // # residual torques +  #GRFs + # GRMs +locations 
...
int F_generic(const T** arg, T** res) {
    ...
    std::vector<T> KCF_in(arg[2], arg[2] + NKCFM);
   ...
   /// Right foot
    ...
    /*/// Knee contact forces
    res[0][ndof ] = value<T>(SumForces_vert_Lat);
    res[0][ndof + 1] = value<T>(SumForces_vert_Med);
    ...
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc] = value<T>(calcn_or_l[i]);   /// calcn_or_l
    }
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc*2] = value<T>(femur_or_r[i]);   /// femur_or_r
    }
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc*3] = value<T>(femur_or_l[i]);   /// femur_or_l
    }
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc*4] = value<T>(hand_or_r[i]);    /// hand_or_r
    }
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc*5] = value<T>(hand_or_l[i]);   /// hand_or_l
    }
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc*6] = value<T>(tibia_or_r[i]);  /// tibia_or_r
    }
    for (int i = 0; i < nc; ++i) {
        res[0][i+ndof + nc * 2 + nc*7] = value<T>(tibia_or_l[i]);  /// tibia_or_l
    }
...
int main() {
    ...
    const Recorder* Recorder_arg[n_in] = { x,u, KCF_in };
