...
int F_generic(const T** arg, T** res) {
	...
	OpenSim::WeldJoint* weld;

    // OpenSim model: initialize components
    /// Model
	model = new OpenSim::Model();
    /// Body specifications
	crane = new OpenSim::Body("crane", 1500.0, Vec3(0),
            Inertia(12500.0,125.0,12500.0,0,0,0));
	cable = new OpenSim::Body("cable", 20.0, Vec3(0),
            Inertia(106.0, 0.1, 106.0, 0, 0, 0));
	ball = new OpenSim::Body("ball", 200.0, Vec3(0),
		Inertia(1.0, 1.0, 1.0, 0, 0, 0));
    /// Joint specifications
	SpatialTransform st_joint1;
	st_joint1[0].setCoordinateNames(OpenSim::Array<std::string>("r1_z", 1, 1));
	st_joint1[0].setFunction(new LinearFunction(1, 3.14159));
	st_joint1[0].setAxis(Vec3(0, 0, 1));
	st_joint1[1].setCoordinateNames(OpenSim::Array<std::string>("r2_x", 1, 1));
	st_joint1[1].setFunction(new LinearFunction(1, 0));
	st_joint1[1].setAxis(Vec3(1, 0, 0));
	//st_joint1[2].setCoordinateNames(OpenSim::Array<std::string>("r1_x", 1, 1));
	//st_joint1[2].setFunction(new LinearFunction());
	st_joint1[2].setAxis(Vec3(0, 1, 0));
	joint1 = new CustomJoint("joint1", model->getGround(), Vec3(0), Vec3(0),
            *crane, Vec3(0,5.0,0), Vec3(0),st_joint1);
	SpatialTransform st_joint2;
	st_joint2[0].setCoordinateNames(OpenSim::Array<std::string>("r3_z", 1, 1));
	st_joint2[0].setFunction(new LinearFunction(1, 3.14159));
	st_joint2[0].setAxis(Vec3(0, 0, 1));
	st_joint2[1].setCoordinateNames(OpenSim::Array<std::string>("r4_x", 1, 1));
	st_joint2[1].setFunction(new LinearFunction(1, 0));
	st_joint2[1].setAxis(Vec3(1, 0, 0));
	st_joint2[2].setCoordinateNames(OpenSim::Array<std::string>("r5_y", 1, 1));
	st_joint2[2].setFunction(new LinearFunction(1, 0));
	st_joint2[2].setAxis(Vec3(0, 1, 0));
	joint2 = new CustomJoint("joint2", *crane, Vec3(0, -5, 0), Vec3(0),
            *cable, Vec3(0, 4, 0), Vec3(0), st_joint2);
	weld = new WeldJoint("weld", *cable, Vec3(0, -4, 0), Vec3(0),
		*ball, Vec3(0), Vec3(0));
