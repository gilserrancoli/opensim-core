...
int F_generic(const T** arg, T** res) {
    ...
    /// Body specifications
	pend1 = new OpenSim::Body("pend1", 300, Vec3(0),
            Inertia(1000, 13.5, 1000, 0, 0, 0));
	pend2 = new OpenSim::Body("pend2", 200, Vec3(0),
			Inertia(434, 9, 434, 0, 0, 0));
	pend3 = new OpenSim::Body("pend3", 100, Vec3(0),
		Inertia(217, 4.5, 217, 0, 0, 0));

    /// Joint specifications
	SpatialTransform st_pin1;
	st_pin1[0].setCoordinateNames(OpenSim::Array<std::string>("r1_y", 1, 1));
	st_pin1[0].setFunction(new LinearFunction());
	st_pin1[0].setAxis(Vec3(0, 1, 0));
	st_pin1[1].setAxis(Vec3(1, 0, 0));
	st_pin1[2].setCoordinateNames(OpenSim::Array<std::string>("r2_z", 1, 1));
	st_pin1[2].setFunction(new LinearFunction());
	st_pin1[2].setAxis(Vec3(0, 0, 1));
	pin1 = new CustomJoint("pin1", model->getGround(), Vec3(0,0,0),
		Vec3(0), *pend1, Vec3(0, -1.5,0), Vec3(0,0,0),st_pin1);

	pin2 = new PinJoint("pin2", *pend1, Vec3(0, 1.5, 0),
		Vec3(0), *pend2, Vec3(0, -1.25, 0), Vec3(0));

	pin3 = new PinJoint("pin3", *pend2, Vec3(0, 1.25, 0),
		Vec3(0), *pend3, Vec3(0, -1.25, 0), Vec3(0));
