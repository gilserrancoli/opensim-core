...
int F_generic(const T** arg, T** res) {
    ...
    /// Body specifications
	pend1 = new OpenSim::Body("pend1", 2.0, Vec3(0),
            Inertia(0.1067,0.01,0.1067,0,0,0));
	pend2 = new OpenSim::Body("pend2", 2.0, Vec3(0),
            Inertia(0.1067, 0.01, 0.1067, 0, 0, 0));
    /// Joint specifications
	/// Joint specifications
	SpatialTransform st_pin1;
	st_pin1[2].setCoordinateNames(OpenSim::Array<std::string>("r1_z", 1, 1));
	st_pin1[2].setFunction(new LinearFunction(1, 3.14159));
	st_pin1[2].setAxis(Vec3(0, 0, 1));
	st_pin1[3].setCoordinateNames(OpenSim::Array<std::string>("r1_x", 1, 1));
	st_pin1[3].setFunction(new LinearFunction());
	st_pin1[3].setAxis(Vec3(1, 0, 0));
	pin1 = new CustomJoint("pin1", model->getGround(), Vec3(0), Vec3(0),
            *pend1, Vec3(0,0.4,0), Vec3(0),st_pin1);
	pin2 = new PinJoint("pin2", *pend1, Vec3(0, -0.4, 0), Vec3(0),
            *pend2, Vec3(0, 0.4, 0), Vec3(0));
    /// Add bodies and joints to model
	model->addBody(pend1);      model->addBody(pend2);
	model->addJoint(pin1);         model->addJoint(pin2);
