...
constexpr int NR = ndof+4;    // # residual torques + 4 GRF (2 rear x and y, 2 front x and y)
...
int F_generic(const T** arg, T** res) {
	...
	Vec3 locSphere_1_r = Vec3(0, 0, 0);
	Vec3 locSphere_2_r = Vec3(0.3, 0, 0);
	/// Parameters
	Real radiusSphere = 0.02;
	Real stiffness = 1000000;
	Real dissipation = 2.0;
	Real staticFriction = 0.8;
	Real dynamicFriction = 0.8;
	Real viscousFriction = 0.5;
	Real transitionVelocity = 0.2;
	Real offset = 0.0;
	...
	// Compute contact forces
	 /// Right foot
	Array<Real> Force_values_1_r = HC_1_r->getRecordValues(*state);
	Array<Real> Force_values_2_r = HC_2_r->getRecordValues(*state);
	...
	for (int i = 0; i < 2; ++i) res[0][ndof + i] =
		value<T>(GRF_1_r[1][i]);
	for (int i = 0; i < 2; ++i) res[0][ndof + 2 + i] =
		value<T>(GRF_2_r[1][i]);
