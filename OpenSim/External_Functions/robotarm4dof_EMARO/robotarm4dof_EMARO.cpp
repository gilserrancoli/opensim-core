...
int F_generic(const T** arg, T** res) {
	...
	OpenSim::Body* bar3;
	OpenSim::Body* bar4;
	OpenSim::Body* end_effector;
	
    /// Joints
	OpenSim::WeldJoint* w1;
	OpenSim::CustomJoint* pin1;
	OpenSim::CustomJoint* pin2;
	OpenSim::CustomJoint* pin3;
	OpenSim::PinJoint* pin4;

	// OpenSim model: initialize components
    /// Model
	model = new OpenSim::Model();
    /// Body specifications
	bar1 = new OpenSim::Body("bar1", 5, Vec3(0),
            Inertia(0.1, 0.001, 0.1, 0, 0, 0));
	bar2 = new OpenSim::Body("bar2", 2.0, Vec3(0),
			Inertia(0.04, 0.004, 0.04, 0, 0, 0));
	bar3 = new OpenSim::Body("bar3", 2.0, Vec3(0),
            Inertia(0.04, 0.004, 0.04, 0, 0, 0));
	bar4 = new OpenSim::Body("bar4", 2.0, Vec3(0),
            Inertia(0.04, 0.004, 0.04, 0, 0, 0));
	end_effector = new OpenSim::Body("end_effector", 1.0, Vec3(0),
		Inertia(0.02, 0.002, 0.02, 0, 0, 0));
	

    /// Joint specifications
	w1 = new WeldJoint("w1", model->getGround(), Vec3(0), Vec3(0),
		    *bar1, Vec3(0,0.4,0), Vec3(0,0,3.14));
	SpatialTransform st_pin1;
	st_pin1[0].setAxis(Vec3(0, 0, 1));
	st_pin1[1].setAxis(Vec3(1, 0, 0));
	st_pin1[2].setCoordinateNames(OpenSim::Array<std::string>("r1_y", 1, 1));
	st_pin1[2].setFunction(new LinearFunction());
	st_pin1[2].setAxis(Vec3(0, 1, 0));
	pin1 = new CustomJoint("pin1", *bar1, Vec3(0,-0.4,0),
		Vec3(0), *bar2, Vec3(0,0.3,0), Vec3(0,0,1.5708),st_pin1);

	SpatialTransform st_pin2;
	st_pin2[0].setAxis(Vec3(0, 0, 1));
	st_pin2[1].setCoordinateNames(OpenSim::Array<std::string>("r2_x", 1, 1));
	st_pin2[1].setFunction(new LinearFunction());
	st_pin2[1].setAxis(Vec3(1, 0, 0));
	st_pin2[2].setAxis(Vec3(0, 1, 0));
	pin2 = new CustomJoint("pin2", *bar2, Vec3(0, -0.3, 0),
		Vec3(0), *bar3, Vec3(0, 0.15, 0), Vec3(0, 0, 0),st_pin2);

	SpatialTransform st_pin3;
	st_pin3[0].setAxis(Vec3(0, 0, 1));
	st_pin3[1].setCoordinateNames(OpenSim::Array<std::string>("r3_x", 1, 1));
	st_pin3[1].setFunction(new LinearFunction());
	st_pin3[1].setAxis(Vec3(1, 0, 0));
	st_pin3[2].setAxis(Vec3(0, 1, 0));
	pin3 = new CustomJoint("pin3", *bar3, Vec3(0, -0.15, 0),
		Vec3(0), *bar4, Vec3(0, 0.25, 0), Vec3(0, 0, 1.5708), st_pin3);

	pin4 = new PinJoint("pin4", *bar4, Vec3(0, -0.25, 0),
		Vec3(0), *end_effector, Vec3(0, 0.25, 0), Vec3(0));
